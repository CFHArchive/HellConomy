...
public class SimplisticEconomy extends Converter {
  ...
  public void mysql() throws InvalidDatabaseImport {
    ...
    try(ResultSet results = db.getConnection().createStatement().executeQuery("SELECT player_name, balance FROM " + config.getString("storage.table") + ";")) {

      final Currency currency = HellConomy.currencyManager().get(HellConomy.instance().getDefaultWorld());
      while(results.next()) {
        Converter.convertedAdd(results.getString("player_name"),
